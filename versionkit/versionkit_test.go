...
func TestParseSemanticVersion(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedPatch:     3,
			expectedPreRelease: "",
			expectedBuild:     "build1",
			expectError:       false,
		},
		{
			name:              "version with pre-release and build metadata",
			input:             "1.2.3-alpha1+build1",
			expectedMajor:     1,
			expectedMinor:     2,
			expectedPatch:     3,
			expectedPreRelease: "alpha1",
			expectedBuild:     "build1",
			expectError:       false,
		},
		{
			name:              "version with dots in pre-release should fail",
	...
	tests := []struct {
		...
		},
		{
			...
			version: SemanticVersion{
				...
				MinorVersion:  2,
				PathVersion:   3,
				BuildMetadata: "build1",
			},
			expected: "1.2.3+build1",
		},
		{
			name: "version with pre-release and build metadata",
			version: SemanticVersion{
				...
				MinorVersion:      2,
				PathVersion:       3,
				PreReleaseVersion: "alpha1",
				BuildMetadata:     "build1",
			},
			expected: "1.2.3-alpha1+build1",
		},
		{
			name: "zero version",
			version: SemanticVersion{
		...
		},
		{
			...
			version: SemanticVersion{
				...
				MinorVersion:      0,
				PathVersion:       0,
				PreReleaseVersion: "rc123",
			},
			expected: "2.0.0-rc123",
		},
		{
